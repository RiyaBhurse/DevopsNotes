...
So we moved to microservices because:
It’s faster to develop and deploy features.
Teams can work independently.
The system becomes more resilient — if one service crashes, the rest keep running.
It’s easier to scale specific parts of your system.
...
Each one has one job. If the cashier goes on break, the chef still cooks — that’s microservices.

In contrast, a monolithic restaurant would be one person doing everything — if they fall sick, everything stops!

🖥️ 5. Normal technical example
...
We’ll break this into parts so you understand how microservices talk, connect, and scale in real-world systems.

🧭 1. DNS (Domain Name System)
What it is:
DNS converts a domain name (like netflix.com) into an IP address (like 192.168.1.5) that computers understand.
Why we need it:
In microservices, you can’t hardcode IPs — services come and go dynamically (especially in Kubernetes). DNS helps them find and talk to each other easily by name.
Example:
Instead of calling http://10.12.3.45:8080/api, a service can just call http://payment-service/api.
Real-life analogy:
It’s like your phone contacts list — you don’t memorize everyone’s number; you just type their name. DNS does the same for services.


⚖️ 2. Load Balancers
What it is:
A load balancer distributes incoming traffic across multiple instances of the same service.
Why we need it:
If your app has 5 instances of User Service, a load balancer ensures that no single instance is overloaded.
Where it’s used:
Every big system — Netflix, Amazon, Instagram — has thousands of services running behind load balancers.
Example:
...
⚡ 1. Queues in Asynchronous Communication — What Are They & How Do They Help?
...
⚙️ 2. What is an Artifact & Config?
...
Imagine now that one of the existing Order Service instances crashes unexpectedly (say due to memory overflow).
...
You have a client (like a web app) → calling microservices inside a distributed system.
   ...
   🌐 API Gateway / Load Balancer
          │
          ▼
   📜 Service Registry (list of all services + IPs)
          │
